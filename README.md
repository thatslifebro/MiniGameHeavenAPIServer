...
* 토큰 유효 기간은 (3분) -> 하이브 로그인후 게임에 로그인하는 과정이 오래걸리지 않음.
...
1. 토큰이 redis에 저장되어 있고 유효기간 내라면 추가 인증 없이 고유번호를 이용해 로그인.
1. 아니라면 게임 서버는 하이브 서버에 토큰 검증을 받는다.
1. 유효 하다면 고유번호를 이용해 로그인
- 첫 로그인 사용자는 유저 등록

## 유저 등록

**컨텐츠 설명**
- 처음 로그인한 유저에 대해 닉네임 정보를 받아 등록합니다.

**로직**
1. 고유번호와 토큰, 닉네임을 받아 기본정보를 생성합니다.
